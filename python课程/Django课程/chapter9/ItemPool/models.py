...
class testItem(models.Model):  # 试题模型
    ...
    type = models.ForeignKey(itemType, on_delete=models.CASCADE, verbose_name='试题类型')  # 外键，关联试题类型
...
class paperTemplate(models.Model):  # 试卷模板模型
    ...
    typeOneScore = models.DecimalField(default=1, max_digits=3, decimal_places=1, verbose_name='单项选择题分值')
    typeTwoCount = models.PositiveSmallIntegerField(default=3, verbose_name='基本操作题数量')
    typeTwoScore = models.DecimalField(default=5, max_digits=3, decimal_places=1, verbose_name='基本操作题分值')
    typeThreeCount = models.PositiveSmallIntegerField(default=2, verbose_name='简单应用题数量')
    typeThreeScore = models.DecimalField(default=12.5, max_digits=3, decimal_places=1, verbose_name='简单应用题分值')
    typeFourCount = models.PositiveSmallIntegerField(default=1, verbose_name='综合应用题数量')
    typeFourScore = models.DecimalField(default=20, max_digits=3, decimal_places=1, verbose_name='综合应用题分值')
...
class paperContent(models.Model):  # 试卷内容模型，保存根据试卷选项生成的试卷
    ...
    template = models.ForeignKey(paperTemplate, on_delete=models.CASCADE, verbose_name='试卷模板')  # 外键，关联试卷模板
